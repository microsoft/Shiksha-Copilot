...
class RegenQueryGenerator:
    ...
    def generate_synthesis_query(
        ...
        if self.lp_gen_input.lp_level == LPLevel.SUBTOPIC:
            synthesis_query = (
                f"You are regenerating the '{section_title}' section of a lesson plan. "
            ...
            synthesis_query = (
                f"You are regenerating the '{section_title}' section of a lesson plan. "
                f"Following is the chapter title and associated learning outcomes:\n\n"
                f"{retrieval_query}"
            )
        ...
        if dependencies:
            ...
            for section_title, content in dependencies.items():
                if section_title.startswith(PREVIOUSLY_GENERATED_DAG_NODE_TITLE_PREFIX):
                    previous_generation_dependency_str = f"# {section_title}\n{content}"
                else:
                    if not current_generation_dependencies_str:
                        current_generation_dependencies_str = ""
                    current_generation_dependencies_str += (
                        f"# Section: {section_title}\n{json.dumps(content)}\n\n"
                    )

            if current_generation_dependencies_str:
                synthesis_query += (
                    "\nThe content of this section depends on the following previously generated sections:\n"
                    "```\n"
                    f"{current_generation_dependencies_str}\n"
                    "```"
                )
            if previous_generation_dependency_str:
                synthesis_query += (
                    "\nBelow is the previously generated content for this section, along with any USER feedback provided for its regeneration (if available):\n"
                    "```\n"
                    f"{previous_generation_dependency_str}\n"
                    "```"
                    "\n**IMPORTANT: Carefully review and incorporate the USER feedback (if any) when regenerating this section. Ensure all feedback points are addressed.**\n"
                )
