...
class PDFSplitter:
    ...
    def split(self, page_ranges: List[Tuple[int, int]], output_dir: str = None, page_offset: int = 0) -> List[str]:
        """
        ...
        for page_range in page_ranges:
            ...
            if adjusted_start < 1:
                raise ValueError(f"With offset {page_offset}, start page {start_page} would be less than 1")
        ...
        for i, page_range in enumerate(adjusted_page_ranges):
            ...
            output_filename = f"{filename_without_ext}_pages_{original_start}-{original_end}{ext}"
    ...
    def split_by_toc(self, toc: TableOfContent, output_dir: str = None, page_offset: int = 0) -> Dict[str, str]:
        """
        ...
        for section in toc.sections:
            ...
            # Ensure pages are within valid range
            if start_page < 1:
                raise ValueError(f"With offset {page_offset}, start page {section.page_range.start_page} would be less than 1")
            ...
            # Create sanitized section name for filename (remove characters that are invalid in filenames)
            safe_section_name = ''.join(c if c.isalnum() or c in ' _-' else '_' for c in section_name)
            safe_section_name = safe_section_name.strip().replace(' ', '_')
            
            # Create output filename
            output_filename = f"{safe_section_name}{ext}"
            output_path = os.path.join(output_dir, output_filename)
