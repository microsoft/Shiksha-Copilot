...
export class ResourcePptGeneratorService {
  ...
  constructor(
    ...
    private pptUtilityService: PptUtilityService
  ) {}

  generatePpt(resources: any, formValues: any,fileName:any) {
  ...
  addSlidesForResources(pptx: any, resources: any) {
    ...
    resources?.forEach((resource: any) => {
      ...
      // Add section title
      slide.addText(this.translateService.instant(resource.title || 'Resource'), {
        x: marginX,
        y: 0.5,
        fontSize: 20,
        bold: true,
      ...
      switch (resource.outputFormat) {
        case 'plain_text': {
          const lines = (resource.content || "").split('\n');
          rows = lines.map((line: string) => [this.pptUtilityService.parseTextForMarkdown(line)]);
          break;
        }

        case 'json_1': { // Question Bank
          resource.content?.forEach((difficultyBlock: any) => {
            difficultyBlock.content?.forEach((section: any) => {
              const typeLabel = this.translateService.instant(section.type?.toUpperCase() || 'Section');
              rows.push([`📘 ${typeLabel}`]);

              section.questions?.forEach((q: any, idx: number) => {
                const questionText = `${this.translateService.instant('Q')}${idx + 1}: ${this.pptUtilityService.parseText(q.question)}`;
                rows.push([questionText]);

                q.options?.forEach((opt: string) => {
                  rows.push([`• ${this.pptUtilityService.parseText(opt)}`]);
                });
        ...
        case 'json_2': { // Real World Scenarios
          resource.content?.forEach((difficultyBlock: any) => {
            difficultyBlock.content?.forEach((rw: any) => {
              rows.push([
                `${this.translateService.instant('Title')}: ${this.pptUtilityService.parseText(rw.title)}`
              ]);
              rows.push([
                `${this.translateService.instant('Question')}: ${this.pptUtilityService.parseText(rw.question)}`
              ]);
              rows.push([
                `${this.translateService.instant('Description')}: ${this.pptUtilityService.parseText(rw.description)}`
              ]);
              rows.push(['']); // spacing
            });
          });
        ...
        case 'json_3': { // Activities
          resource.content?.forEach((activity: any) => {
            const fields = ['title', 'preparation', 'required_materials', 'obtaining_materials', 'recap'];

            fields.forEach(field => {
              const label = this.translateService.instant(this.toTitleCase(field.replace(/_/g, ' ')));
              const value = this.pptUtilityService.parseText(activity[field]);
        ...
        default: {
          rows.push([this.translateService.instant('Unsupported Format')]);
        }
      }

      // Add content as auto-paginated table
      slide.addTable(rows, {
        x: marginX,
        y: 1.2,
        w: 9,
