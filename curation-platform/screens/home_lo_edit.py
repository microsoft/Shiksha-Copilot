...
from data.api import get_all_chapter_docs_edited, get_all_chapter_docs_unedited, get_chapters_summary_doc
from data.chapter_lo_subtopic_models import UserRoleEnum
from navigation import Navigation as nav
from screens import edit_chapter_lo_subtopic, register_new_teacher
...
def app():
    ...
    if user.role == UserRoleEnum.ADMIN:
        ...
        if st.sidebar.button("User Registration"):
            nav.set_current_page(register_new_teacher)
    
    if st.sidebar.button("Logout", key="Logout"):
        logout()
        st.toast("Logged out")
        
    st.header("Chapter Subtopics and Learning Outcomes Editing", divider="grey")
    
    
    with st.spinner('Getting chapter details...'):
        data = SM.chapter_summary.get()
        if data == None:
            data = get_chapters_summary_doc()
            SM.chapter_summary.set(data)
                ...
                if st.session_state['selected_subject']:
                    if user.role == UserRoleEnum.ADMIN:
                        with st.spinner("Fetching chapters..."):
                            ...
                            edited_chap_ids = [chap.id for chap in unedited_chapters if chap.isEdited]
                            edited_chapters = get_all_chapter_docs_edited(edited_chap_ids)
                        ...
                        for chapter in chapters:
                            ...
                            cols[2].write('Yes' if chapter.isEdited else 'No')
                            if cols[3].button("View", key=str(chapter.id)):
                                ...
                                SM.chosen_chapter_name.set(chosen_chapter_name)
                                SM.chosen_chapter_id.set(chapter.id)
                                nav.set_current_page(edit_chapter_lo_subtopic)
                    else:
                        chapter_ids = data[board][medium][str(grade)][subject]
                        ...
                        if chosen_chapter_name in chapter_option_to_id:
                            if st.button("Edit", type="primary"):
                                SM.chosen_chapter_name.set(chosen_chapter_name)
                                SM.chosen_chapter_id.set(chapter_option_to_id[chosen_chapter_name])
                                nav.set_current_page(edit_chapter_lo_subtopic)